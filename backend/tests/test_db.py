...
@pytest.fixture()
def db():
    db = SQLiteLocationDatabase("test_locations.db")
...
def test_find_services(db):
    ...
    assert len(res) == 2
    assert res[0].service_name == "test" or res[0].service_name == "test2"
    assert res[1].service_name == "test" or res[1].service_name == "test2"
