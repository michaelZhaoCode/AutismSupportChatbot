...
class Chatbot:
    ...
    def _generate_filter(self, prompt: str, username: str, usertype: str) -> str:
        """
        Generate a filter chat response based on the given prompt and user's chat history.
        ...
        """
        chat_history = self.chat_history.retrieve_chat_history(username)
        prompt_format = self._load_prompt(usertype.lower(), 'filter').format(username, prompt)
        print("Getting filter response")
        response = self.co.chat(
            model="command-r-plus",
            message=prompt_format,
            chat_history=chat_history
    ...
    def _classify(self, prompt):
        ...
        #     print(x.document.text, x.relevance_score)
    ...
    def chat(self, prompt: str, username: str, usertype: str) -> str:
        ...
        if choice == 'rag':
            ...
            response = self._generate_filter(prompt, username, usertype)
